...
export default function VideoPlayer({
  ...
  onViewFullVideo,
  onCopyLink,
  productRefreshTrigger  // ← NEW PROP: Triggers refresh
}) {
  const videoRef = useRef(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [videoError, setVideoError] = useState(false);
  ...
  // ← THIS IS THE ONLY NEW useEffect — refreshes when any modal closes
  useEffect(() => {
  ...
  const renderProductImages = () => {
    if (isLoadingProducts) return <div style={{ color: 'white', fontSize: '0.6rem' }}>Loading...</div>;
    if (savedProducts.length === 0) return <div style={{ color: 'white', fontSize: '0.75rem', fontWeight: '500' }}>Tag Products</div>;
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
        {productsToShow.map((product, index) => (
          <div key={product.video_product_id || product.id} style={{ width: '20px', height: '20px', borderRadius: '4px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.3)' }}>
            {product.image_url ? (
              <img src={product.image_url} alt={product.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ) : (
              <div style={{ width: '100%', height: '100%', background: '#3b82f6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.5rem', fontWeight: 'bold' }}>P</div>
            )}
          </div>
        ))}
        {savedProducts.length > 3 && (
          <div style={{ width: '20px', height: '20px', background: 'rgba(255,255,255,0.2)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.5rem', fontWeight: 'bold' }}>
            +{savedProducts.length - 3}
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      {bulkDeleteMode && (
        <div className="checkbox-wrapper">
          <input type="checkbox" checked={isSelected} onChange={onSelect} className="bulk-checkbox" />
        </div>
      )}

      {!bulkDeleteMode && (
        <button onClick={handleEditClick} style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(0, 0, 0, 0.6)', border: 'none', borderRadius: '6px', color: 'white', padding: '6px 10px', fontSize: '0.7rem', fontWeight: '500', cursor: 'pointer', zIndex: 10, backdropFilter: 'blur(4px)' }}>
          <img src="https://cdn-icons-png.flaticon.com/512/3597/3597075.png" alt="Edit" style={{ width: '12px', height: '12px', filter: 'invert(1)' }} />
        </button>
      )}

      {showEditMenu && !bulkDeleteMode && (
        <div style={{ position: 'absolute', top: '40px', right: '8px', background: 'rgba(0, 0, 0, 0.8)', borderRadius: '8px', padding: '8px', zIndex: 20, backdropFilter: 'blur(8px)', border: '1px solid rgba(255, 255, 255, 0.2)', minWidth: '150px' }}>
          <button onClick={handleViewFullVideo} style={{ width: '100%', background: 'transparent', border: 'none', color: 'white', padding: '8px 12px', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src="https://cdn-icons-png.flaticon.com/512/2991/2991937.png" alt="Video" style={{ width: '14px', height: '14px', filter: 'invert(1)' }} />
            View Full Video
          </button>
          <button onClick={handleCopyLink} style={{ width: '100%', background: 'transparent', border: 'none', color: 'white', padding: '8px 12px', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src="https://cdn-icons-png.flaticon.com/512/684/684908.png" alt="Copy Link" style={{ width: '14px', height: '14px', filter: 'invert(1)' }} />
            Copy Link
          </button>
          <button onClick={handleTagProducts} style={{ width: '100%', background: 'transparent', border: 'none', color: 'white', padding: '8px 12px', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src="https://cdn-icons-png.flaticon.com/512/3917/3917759.png" alt="Tag Products" style={{ width: '14px', height: '14px', filter: 'invert(1)' }} />
            Tag Products
          </button>
        </div>
      ...
      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
        {videoError ? (
          <div style={{ width: '100%', height: '100%', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: theme.mutedText, flexDirection: 'column', gap: '0.5rem' }}>
            <div style={{ fontSize: '2rem' }}>Video unavailable</div>
            <button onClick={() => { setVideoError(false); if (videoRef.current) videoRef.current.load(); }} style={{ background: '#3b82f6', color: 'white', border: 'none', padding: '0.25rem 0.5rem', borderRadius: '4px', fontSize: '0.75rem', cursor: 'pointer' }}>
              Retry
            </button>
          </div>
        ) : (
