...
export function useVideoGallery() {
  ...
  // Add a new state to track when products are saved
  ...
  // Load media files - UPDATED: Now sorts by position
  const loadMediaFiles = async () => {
    try {
      ...
      if (result.success) {
        // Sort media files by position
        const sortedMediaFiles = result.mediaFiles.sort((a, b) => a.position - b.position);
  ...
  // Add a new function to handle arrangement saved callback
  const handleArrangementSaved = () => {
    showToast('Video arrangement saved successfully', 'success');
    loadMediaFiles(); // Refresh to get new positions
  };
  ...
  // Save selected products for video - UPDATED to trigger refresh
  const saveVideoProducts = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Increment the productsSaved counter to trigger refresh in VideoOptionsModal
        setProductsSaved(prev => prev + 1);

        console.log('✅ Products saved successfully! Triggered refresh.');
        ...
        if (showVideoOptions.show && showVideoOptions.video?.id === showProductsModal.video.id) {
          // Force VideoOptionsModal to refresh by triggering a state update
          setShowVideoOptions(prev => ({ ...prev, refresh: Date.now() }));
        }
      } else {
  ...
  return {
    ...
    showVideoPlayer,
    productsModalOpened,
    productsSaved, // Add this to track when products are saved
    ...
    // NEW: Arrangement saved handler
    handleArrangementSaved
  };
}