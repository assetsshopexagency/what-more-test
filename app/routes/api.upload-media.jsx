...
export async function action({ request }) {
  ...
  try {
    ...
    if (file.type.startsWith('video/')) {
      ...
      const { path: inputPath, cleanup: inputCleanup } = await tmpFile({ postfix: path.extname(file.name) });
      await fs.writeFile(inputPath, Buffer.from(originalBuffer));

      // Create temp output file
      const { path: outputPath, cleanup: outputCleanup } = await tmpFile({ postfix: '.mp4' });

      // Compress with higher CRF for more compression
      await new Promise((resolve, reject) => {
        ffmpeg(inputPath)
          .videoCodec('libx264')
          .addOption('-crf', '28')  // Higher CRF for more compression (smaller size)
          .addOption('-preset', 'veryfast')  // Faster encoding
          .addOption('-movflags', 'faststart')  // Optimize for web
          .addOption('-threads', '0')  // Auto threading for speed
          .output(outputPath)
          .on('end', () => {
      ...
      if (compressedSize < originalSize) {
        ...
        file = new File([compressedBuffer], file.name.replace(/\.[^/.]+$/, ".mp4"), {
          type: 'video/mp4',
          lastModified: Date.now()
        });
        console.log('✅ Using compressed file');
      } else {
        // Use original if compression increased size
        file = new File([originalBuffer], file.name, {
          type: file.type,
          lastModified: Date.now()
        });
        console.log('⚠️ Compressed larger than original, using original file');
      }
